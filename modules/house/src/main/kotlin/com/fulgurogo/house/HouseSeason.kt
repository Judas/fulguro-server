...
/**
 * The house calendar. A season runs from 1 September to 30 June and is named after the two years it spans
 * (`"2026-2027"`); July and August are the break, when nothing is scored and the holiday choices are applied.
 *
 * Nothing here reads the database or the clock behind the caller's back: [period] and [seasonName] take the instant to
 * answer for, so the four dates worth checking — 31 August, 1 September, 30 June, 1 July — can be checked by hand.
 */
object HouseSeason {
    ...
    /** July: the first month of the break, and the exclusive end of the season window. */
    private const val VACATION_START_MONTH = 7
     ...
     * Forcing `SEASON` in July therefore opens the paths that a period gates — joining a house, above all — without
     * making July games scorable, since they stay outside the window of the season that just ended.
     */
    ...
    /**
     * The season [now] belongs to. From September on, that is the season starting this year; up to June, the one that
     * started last year.
     *
     * In July and August it names the season that has just ended, and the single `year - 1` branch it shares with
     * January-to-June is not a coincidence. A game played on 30 June and only scanned on 1 July still falls inside that
     * season's window and still scores, while a game played on 15 July never will. "No points outside the season" is a
     * rule about the date of the *game*, not the date of the scan, and this is what keeps the last games of June.
     */
    ...
    /**
     * The half-open window of a season: 1 September 00:00 inclusive, to 1 July 00:00 exclusive. A game counts when
     * `start <= date < end`, which puts the whole of 30 June in and the whole of July out.
     *
